...
#define SmaNewfeatureImageCount 3

@interface SmaNewfeatureViewController ()<UIScrollViewDelegate>
@property (nonatomic, weak) UIPageControl *pageControl;
@end

@implementation SmaNewfeatureViewController
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = SmaNewfeatureImageCount;
...
- (void)setupScrollView
{
    ...
    for (int index = 0; index<SmaNewfeatureImageCount; index++) {
        ...
        // 在最后一个图片上面添加按钮
        if (index == SmaNewfeatureImageCount - 1) {
            //[self setupLastImageView:imageView];
        }
    }
    
    // 3.设置滚动的内容尺寸
    scrollView.contentSize = CGSizeMake(imageW * SmaNewfeatureImageCount, 0);
...
- (void)start
{
    ...
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:[[SmaLoginViewController alloc] init]];
