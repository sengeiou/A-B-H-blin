...
- (void)viewDidLoad
{
    ...
    MyLog(@"%@----------%@",lastVersion,currentVersion);
        ...
        if (!lastVersion) {
            UIAlertView *aler = [[UIAlertView alloc] initWithTitle:nil message:SmaLocalizedString(@"alear_firstCam") delegate:self cancelButtonTitle:SmaLocalizedString(@"clockadd_confirm") otherButtonTitles:nil, nil];
            [aler show];
