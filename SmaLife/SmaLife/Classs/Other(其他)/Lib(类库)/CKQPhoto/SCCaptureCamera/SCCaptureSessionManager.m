...
//        [SCCommon saveImageToPhotoAlbum:image];
        ...
        CGFloat squareLength = SC_APP_SIZE.width;
          CGFloat headHeight = _previewLayer.bounds.size.height ;//- squareLength;//_previewLayer的frame是(0, 44, 320, 320 + 44) SC_APP_SIZE.height;//
        CGSize size = CGSizeMake(squareLength * 2, headHeight * 2);
...
- (void)switchFlashMode:(UIButton*)sender {
    ...
    if (!captureDeviceClass) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:SmaLocalizedString(@"alert_title") message:SmaLocalizedString(@"without_camera") delegate:nil cancelButtonTitle:SmaLocalizedString(@"Sure") otherButtonTitles: nil];
        [alert show];
        return;
    }
...
//        }
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:SmaLocalizedString(@"alert_title") message:SmaLocalizedString(@"without_flash") delegate:nil cancelButtonTitle:SmaLocalizedString(@"clockadd_confirm") otherButtonTitles: nil];
        [alert show];
    }
