...
@interface SmaLosePwdController ()<SecondViewControllerDelegate>

@end

@implementation SmaLosePwdController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title=SmaLocalizedString(@"modify_password");
...
-(void)loadFrome
{
    ...
    verifyCode.frame = CGRectMake(46,pwdLine.frame.origin.y+pwdLine.frame.size.height+19, tfw-80, 32);
    //userPwdField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"密码" attributes:attrs];
    ...
    ////    // 3.设置文字
    [startButton setTitle:SmaLocalizedString(@"modify_password") forState:UIControlStateNormal];
...
- (void)regBtnClick {
    ...
    [MBProgressHUD showMessage:SmaLocalizedString(@"reset_pass")];
    SmaAnalysisWebServiceTool *webservice=[[SmaAnalysisWebServiceTool alloc]init];
          NSString *mobile=[NSString stringWithFormat:@"%@%@",[[areaCodeField.text stringByReplacingOccurrencesOfString:@"+" withString:@"00"] isEqualToString:@"0086"]?@"":[areaCodeField.text stringByReplacingOccurrencesOfString:@"+" withString:@"00"],_phoneCode.text];
     [webservice acloudResetPasswordWithAccount:mobile verifyCode:self.verifyCode.text password:self.pwdCode.text success:^(id uid) {
          [MBProgressHUD hideHUD];
         [MBProgressHUD showMessage:SmaLocalizedString(@"change_passsucc")];
         dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
          ...
          [MBProgressHUD hideHUD];
         [MBProgressHUD showError:SmaLocalizedString(@"change_passfail")];
     }];
    }
